...
/* psuedo code
    take in an array of letters,
    parse through the letters to count each instence of a lettter
    assign the letter a key with the value of the count
    return new array/dict
...
function oddOccurrencesInArray(arr) {
    let dict = makeFrequencyTable(arr);
    for (let i = 0; i < arr.length; i++) {
        if (dict[arr[i]] % 2 == 1) {
            return arr[i]
        }
