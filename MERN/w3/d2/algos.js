...
function findObjects(criteria, collection) {
    ...
    for (key in collection) {
        ...
        for (part in criteria) {
            if (collection[key][part] !== criteria[part]) {
                isValid = false
            }
        }
        if (collection[key][part] === criteria[part] && isValid == true) {
            expected[key] = collection[key]
        }
    }
    return expected
}
...
/**
 * Psuedo Code:
 * Need a copy of original objects: (.map function)
 * Access the attributes of the object, and change it directly.
